...
// Primary Endpoint: Generate Full Multi-Agent Startup Evaluation
app.post("/api/analyze-startup", async (req, res) => {
  try {
    const { idea, targetAudience, countryMarket, budget, goals } = req.body;
...
`;
    ...
    const report = {
      ...
      input: { idea, targetAudience, countryMarket, budget, goals },
      ...parsedData
    };
...
// Interactive Agent Q&A Endpoint
app.post("/api/agent-chat", async (req, res) => {
  try {
    const { agentRole, startupContext, userQuestion, chatHistory } = req.body;

    if (!agentRole || !userQuestion) {
      return res.status(400).json({ error: "agentRole and userQuestion are required." });
    }
...
`;
    ...
    return res.json({
      reply: response.text || "I have analyzed your request based on our current business parameters."
    });
  } catch (err: any) {
